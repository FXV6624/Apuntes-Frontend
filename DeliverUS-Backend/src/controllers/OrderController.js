...
const update = async function (req, res) {
  // Inicia una transacción en la base de datos para asegurar la consistencia
  const transaction = await sequelizeSession.transaction()

  try {
    // Actualiza los datos del pedido en la base de datos con los nuevos valores del cuerpo de la solicitud
    await Order.update(req.body, { where: { id: req.params.orderId } }, { transaction })

    // Busca el pedido actualizado por su ID
    let updatedOrder = await Order.findByPk(req.params.orderId)

    // Elimina todos los productos asociados al pedido (prepara para agregar nuevos productos)
    await updatedOrder.setProducts([], { transaction })

    // Obtiene las líneas de productos con sus precios a partir de la solicitud
    const productLinesWithPrices = await _getProductLinesWithPrices(req.body.products)

    // Calcula el costo total de los productos y los costos de envío para el pedido actualizado
    updatedOrder = await _getOrderWithShippingCostsAndPrice(updatedOrder, productLinesWithPrices)

    // Guarda los productos asociados al pedido en la base de datos usando la transacción
    updatedOrder = await _saveOrderWithProducts(updatedOrder, productLinesWithPrices, transaction)

    // Si todo es exitoso, realiza el commit de la transacción para aplicar los cambios en la base de datos
    await transaction.commit()

    // Envía la respuesta con el pedido actualizado en formato JSON
    res.json(updatedOrder)
  } catch (err) {
    // Si hay un error, revierte la transacción para no dejar datos inconsistentes
    await transaction.rollback()

    // Responde con un error 500 y el mensaje de error
    res.status(500).send(err)
  }
}
