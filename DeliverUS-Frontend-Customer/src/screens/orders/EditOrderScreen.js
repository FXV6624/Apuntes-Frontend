...
export default function EditOrderScreen ({ navigation, route }) {
  ...
  const fetchRestaurantDetail = async () => {
    try {
      ...
      const p = new Map()
      fetchedOrder.products.forEach(element => {
        p.set(element.id, {
          quantity: element.OrderProducts.quantity,
          unityPrice: element.price
        })
      })
      for (const pro of fetchedRestaurant.products) {
        if (!p.has(pro.id)) {
          p.set(pro.id, {
            quantity: 0,
            unityPrice: pro.price
          })
        }
      }
      setProducts(p)
  ...
  const fetchRestaurantDetail2 = async () => {
    try {
      const fetchedOrder = await getOrderDetail(route.params.id)
      const fetchedRestaurant = await getDetail(fetchedOrder.restaurantId)
      const p = new Map()
  ...
  const confirmOrder = async () => {
    try {
      ...
      await update(route.params.id, order)
      showMessage({
